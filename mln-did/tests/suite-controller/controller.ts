...
describe('sol-did controller operations', () => {
  ...
  it('fails to update the "other" controller if it includes an invalid DID (program).', async () => {
    return expect(
      ...
      'Error Code: InvalidOtherControllers. Error Number: 6006. Error Message: Invalid other controllers. Invalid DID format or did:mln:<did>'
    );
  });

  it('fails to update the "other" controller if it includes a did:mln DID (program).', async () => {
    return expect(
      program.methods
        .setControllers(
          {
