...
describe('sol-did resolve and migrate operations', () => {
  ...
  it('can successfully resolve a generative DID', async () => {
    ...
    expect(didDoc).to.deep.equal(
      getGeneratedDidDocument(solKey.publicKey.toBase58(), 'did:mln:localnet:')
    );
  });
  ...
  it('can successfully migrate a legacy DID and close the legacy account', async () => {
    ...
    // why is this failing? Version update? This broke after "@miraland/web3.js went to 1.50.1 in yarn.lock
  ...
  it('cannot migrate if the account is not owned by the legacy did:mln program', async () => {
  ...
  it('updates the verificationMethods of a DID with the correct flags', async () => {
    ...
    expect(doc.authentication).to.be.deep.equal([
      `did:mln:localnet:A2oYuurjzc8ACwQQN56SBLv1kUmYJJTBjwMNWVNgVaT3#default`,
      `did:mln:localnet:A2oYuurjzc8ACwQQN56SBLv1kUmYJJTBjwMNWVNgVaT3#key2`,
    ]);
  ...
  it('can update a did:mln with a DID Document', async () => {
