...
pub fn remove_service(
    ...
    data.services
        ...
        .ok_or_else(|| error!(DidMlnError::ServiceFragmentNotFound))
}
...
pub struct RemoveService<'info> {
    #[account(
        ...
        bump = did_data.bump,
        constraint = did_data.find_authority_constraint(&authority.key(), &borsh::to_vec(&service_id).unwrap(), eth_signature.as_ref(), None).is_some(),
    )]
