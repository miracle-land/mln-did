...
pub fn update(
    ...
    require!(
        ...
        DidMlnError::VmCannotRemoveProtected
    );
...
pub struct Update<'info> {
    #[account(
        ...
        bump = did_data.bump,
        constraint = did_data.find_authority_constraint(&authority.key(), &borsh::to_vec(&update_arg).unwrap(), eth_signature.as_ref(), None).is_some()
    )]
...
pub struct UpdateArg {
    ...
    /// Controller (native) - did:mln:<controller>
