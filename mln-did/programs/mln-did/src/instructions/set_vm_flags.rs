...
pub fn set_vm_flags(
    ...
    require!(vm.is_some(), DidMlnError::VmFragmentNotFound);
    let vm = vm.unwrap();
    ...
    require!(
        ...
        DidMlnError::VmCannotRemoveLastAuthority
    );
...
pub struct SetVmFlagsMethod<'info> {
    #[account(
        ...
        bump = did_data.bump,
        constraint = did_data.find_authority_constraint(&authority.key(), &borsh::to_vec(&flags_vm).unwrap(), eth_signature.as_ref(), flags_vm.get_filter_fragment()).is_some(),
    )]
