...
use crate::{errors::DidMlnError, DidAccount};
use anchor_lang::prelude::*;
use solana_program::account_info::AccountInfo;

/// Will return if given miraland public key, or ethereum address (as derived from an ethereum signature)
...
pub fn is_authority(
    ...
    if did_account.owner == &System::id() {
        ...
        let address = if let Some(did_account_seed_bump) = did_account_seed_bump {
            ...
            // the key must be a miraland pubkey in the generative DID case
    ...
    if !did_data.is_controlled_by(controller_chain.as_slice()) {
        return Err(error!(DidMlnError::InvalidControllerChain));
    }
