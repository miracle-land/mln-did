...
import { SolDid } from '../target/types/mln_did';
...
import { Keypair, PublicKey } from '@miraland/web3.js';
...
(async () => {
  ...
  // write account
  await exec(
    `miraland account ${didData.toBase58()} -ul -o ${fixturePath}did-account-min.json --output json`
  );
  ...
  // write account
  await exec(
    `miraland account ${didData.toBase58()} -ul -o ${fixturePath}did-account-complete.json --output json`
  );
  ...
  // write account
  await exec(
    `miraland account ${legacyAccount.toBase58()} -ul -o ${fixturePath}legacy-did-account-complete.json --output json`
  );
  ...
  // write account
  await exec(
    `miraland account ${unsupportedAccount.toBase58()} -ul -o ${fixturePath}legacy-did-account-wrong-owner.json --output json`
  );
  ...
  // Deploy IDL
  await exec(
    `anchor idl init --filepath ./target/idl/mln_did.json ${DID_SOL_PROGRAM}`
  );
  ...
  await exec(
    `miraland account ${idlAddr.toBase58()} -ul -o ${fixturePath}idl-account.json --output json`
  );
